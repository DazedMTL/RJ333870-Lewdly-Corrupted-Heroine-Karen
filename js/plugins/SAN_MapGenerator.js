...
Game_MapGenerator.prototype.initSymbolTable = function () {
    ...
    this._symbolTable = {
        ...
        space: { refXY: { x: 0, y: 0 }, baseTileId: [], dispChar: '_' },
        room: { refXY: { x: 0, y: 1 }, baseTileId: [], dispChar: '□' },
        room2: { refXY: { x: 1, y: 1 }, baseTileId: [], dispChar: '□' },
        room3: { refXY: { x: 2, y: 1 }, baseTileId: [], dispChar: '□' },
        room4: { refXY: { x: 3, y: 1 }, baseTileId: [], dispChar: '□' },
        room5: { refXY: { x: 4, y: 1 }, baseTileId: [], dispChar: '□' },
        room6: { refXY: { x: 5, y: 1 }, baseTileId: [], dispChar: '□' },
        room7: { refXY: { x: 6, y: 1 }, baseTileId: [], dispChar: '□' },
        pass: { refXY: { x: 0, y: 2 }, baseTileId: [], dispChar: '■' },
        pass2: { refXY: { x: 1, y: 2 }, baseTileId: [], dispChar: '■' },
        pass3: { refXY: { x: 2, y: 2 }, baseTileId: [], dispChar: '■' },
        pass4: { refXY: { x: 3, y: 2 }, baseTileId: [], dispChar: '■' },
        pass5: { refXY: { x: 4, y: 2 }, baseTileId: [], dispChar: '■' },
        pass6: { refXY: { x: 5, y: 2 }, baseTileId: [], dispChar: '■' },
        pass7: { refXY: { x: 6, y: 2 }, baseTileId: [], dispChar: '■' },
        roof: { refXY: { x: 0, y: 3 }, baseTileId: [], dispChar: '#' },
        wall: { refXY: { x: 0, y: 4 }, baseTileId: [], dispChar: '=' },
        rubble: { refXY: { x: 0, y: 5 }, baseTileId: [], dispChar: '＊' },
        start: { refXY: { x: 4, y: 0 }, baseTileId: [], dispChar: '△' },
        goal: { refXY: { x: 5, y: 0 }, baseTileId: [], dispChar: '▽' }//,
...
Game_MapGeneratorRoomAndPass.prototype.makeAdjacentBlockIndexList = function () {
    for (var crntIndex = 0; crntIndex < this._blocks.length - 1; crntIndex++) {
    ...
    p('this._adjacentBlockIndexList')
    p(this._adjacentBlockIndexList)
};

// 部屋作成
Game_MapGeneratorRoomAndPass.prototype.makeRooms = function () {
    this._mapCreater = this._mapCreater || SceneManager._scene._creater;
    ...
    this._blocks.forEach(function (block) {
        ...
        //p(roomW + ' ' + this._maxRoomSize + ' ' + roomH +' ' + this._maxRoomSize);
        var room = this._mapCreater.createRoom(block, this._minRoomSize, roomW, roomH);
        p(room)
        /*
    ...
    this._rooms.forEach(function (room) {
        for (var y = 0; y < room.h; y++) {
            for (var x = 0; x < room.w; x++) {
                this._symbolMap[room.x + x][room.y + y] = 'room2';
            }
        }
    }, this);
};

Game_MapGeneratorRoomAndPass.prototype.findRoomId =
