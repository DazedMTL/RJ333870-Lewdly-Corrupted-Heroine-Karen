...
(function (Nore) {
    ...
    Game_Event.prototype.screenX = function () {
        ...
        if (this._norePositionX === undefined) {
            var e_1 = this.event();
            if (e_1 && e_1.meta['position_x']) {
                this._norePositionX = parseInt(e_1.meta['position_x']);
                if (isNaN(this._norePositionX)) {
                    console.error('position_x の値が不正です: ' + e_1.meta['position_x']);
    ...
    Game_Event.prototype.screenY = function () {
        ...
        if (this._norePositionY === undefined) {
            var e_2 = this.event();
            if (e_2 && e_2.meta['position_y']) {
                this._norePositionY = parseInt(e_2.meta['position_y']);
                if (isNaN(this._norePositionY)) {
                    console.error('position_y の値が不正です: ' + e_2.meta['position_y']);
