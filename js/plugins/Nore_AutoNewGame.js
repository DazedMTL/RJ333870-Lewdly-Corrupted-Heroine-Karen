    ...
    (function (AutoNewGame) {
        ...
        Scene_Boot.prototype.create = function () {
            if (AUTO_CONVERT && Utils.isOptionValid('test')) {
                if (!Nore.Tes) {
                    console.error('Nore_AutoNewGame : autoConvert が ON になっていますが、Nore_Tes プラグインが見つかりません');
                }
                else {
            ...
            if (gameFileData != null) {
                if (gameFileData === '0') {
                    ...
                    DataManager.loadGame(savefileId)
                        .then(function () { return _this.onLoadSuccess(); })
                        .catch(function () { return _this.onLoadFailure(); });
                }
                return;
            }
            else {
                var noreCommonEventId = parseInt(sessionStorage.getItem('noreCommonEventId'));
