...
DataManager.createGameObjects = function () {
    $gameTemp = new Game_RogueTemp();
    $gameSystem = new Game_RogueSystem();
    ...
    $gameParty = new Game_RogueParty();
    $gameTroop = new Game_Troop();
    $gameMap = new Game_Map();
    $gamePlayer = new Game_RoguePlayer();
};
...
DataManager.latestSavefileId = function () {
    const globalInfo = this._globalInfo;
    const validInfo = globalInfo.slice(1, globalInfo.length - 1).filter(x => x);
...
ImageManager.loadBitmap = function (folder, filename, noCache) {
    if (filename) {
        let url = folder + Utils.encodeURI(filename) + ".png";
        return this.loadBitmapFromUrl(url, noCache);
    } else {
        return this._emptyBitmap;
    }
};

ImageManager.loadBitmapFromUrl = function (url, noCache) {
    const cache = url.includes("/system/") ? this._system : this._cache;
    if (!cache[url]) {
        let loadUrl = url;
        if (noCache) {
            loadUrl = loadUrl + '?' + new Date().getTime();
        }
        cache[url] = Bitmap.load(loadUrl);
    }
    return cache[url];
};
...
ImageManager.removeHistory = function (file) {
    const path = "history/" + file + '.png';
    const cache = this._cache;
    if (cache[path]) {
        cache[path].destroy();
        delete cache[path];
    } else {
        //console.error('picture not found:' + file);
    }
};
