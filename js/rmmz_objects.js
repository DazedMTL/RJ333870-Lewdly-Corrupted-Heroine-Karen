...
Game_Screen.prototype.showPicture = function (
    pictureId, name, origin, x, y, scaleX, scaleY, opacity, blendMode, rotation
) {
    const realPictureId = this.realPictureId(pictureId);
    const picture = new Game_Picture();
    picture.show(name, origin, x, y, scaleX, scaleY, opacity, blendMode, rotation);
...
Game_Picture.prototype.show = function (
    name, origin, x, y, scaleX, scaleY, opacity, blendMode, rotation
) {
    this._name = name;
    this._origin = origin;
    this._x = x;
...
Object.defineProperties(Game_BattlerBase.prototype, {
    ...
    mhp: {
        get: function () {
            return Math.min(this.param(0), 999);
        },
        configurable: true
    },
    ...
    mmp: {
        get: function () {
            return Math.min(this.param(1), 99);
        },
        configurable: true
    },
    // ATtacK power
...
Game_Enemy.prototype.selectAllActions = function (actionList) {
    ...
    for (let i = 0; i < this.numActions(); i++) {
        //p(this.selectAction(actionList, ratingZero))
...
Game_Interpreter.prototype.wait = function (duration) {
    if ($gameSwitches.value(34) && isSkipKey()) {
        if (this._fadeOutDuration == 0 && this._fadeInDuration == 0) {
            duration /= 15;
        }
    }
